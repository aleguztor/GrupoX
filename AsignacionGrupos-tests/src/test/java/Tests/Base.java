...
public class Base {
	...
	public static void datos(EntityManager em) {
		...
		em.persist(as);
		em.persist(as1);
		Grupos_Por_AsignaturaPersist(gpa,em);
		AlumnosPersist(alumnos,em);
		MatriculasPersist(mats,em);
		ExpedientesPersist(exps,em);
		GruposPersist(gps,em);
		EncuestasPersist(p,em);
	...
	private static void AlumnosPersist(List<Alumno> al, EntityManager em) {
		for(Alumno a : al) {
			em.persist(a);
		}
	}
	private static void ExpedientesPersist(List<Expediente> exps, EntityManager em) {
		for(Expediente e : exps) {
			em.persist(e);
		}
	}
	private static void GruposPersist(List<Grupo> g, EntityManager em) {
		for(Grupo gr : g) {
			em.persist(gr);
		}
	}
	private static void MatriculasPersist(List<Matricula> mats, EntityManager em) {
		for(Matricula m : mats) {
			em.persist(m);
		}
	}
	private static void EncuestasPersist(List<Encuesta> en, EntityManager em) {
		for(Encuesta e : en) {
			em.persist(e);
		}
	}
	private static void Grupos_Por_AsignaturaPersist(List<Grupos_Por_Asignatura> gpa, EntityManager em) {
